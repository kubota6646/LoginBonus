...
3. **依存関係の追加**
   ...
   - 注: ビルド修正で正しいバージョン形式に更新（5.6.3196 → 5.6.2965）
...
4. **ビルド修正**
   - Plan依存関係のバージョン形式を修正（5.6.3196 → 5.6.2965）
   - JitPackの正しいバージョン形式（ドットで区切る）を使用
   - 存在しないバージョンによるビルドエラーを解消

5. **Plan連携のプレイヤー名表示修正**
   - `PlanDataExtension.consecutiveLoginRanking()`でプレイヤー名が正しく表示されない問題を修正
   - `hasPlayedBefore()`チェックを追加して、サーバーに参加したことがあるプレイヤーのみ名前を表示
   - 未参加プレイヤーはUUIDの短縮版（最初の8文字）を表示
